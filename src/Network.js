...
export class Network {
  ...
  */
  toDot(edgeConnection) {
    ...
    let code = 'digraph nn {\n    rankdir = BT\n';
    let layers = [this.layers.input].concat(this.layers.hidden, this.layers.output);
    for (let i = 0; i < layers.length; i++) {
      for (let j = 0; j < layers[i].connectedTo.length; j++) { // projections
  ...
  standalone() {
    ...
    for (let i = 0; i < data.activate.length; i++) { // shouldn't this be layer?
